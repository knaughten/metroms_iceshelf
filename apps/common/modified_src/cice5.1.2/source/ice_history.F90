...
!     call broadcast_scalar (f_example, master_task)
      ...
      call broadcast_scalar (f_freshi, master_task)
      call broadcast_scalar (f_freshi_ai, master_task)
      call broadcast_scalar (f_fsalt, master_task)
      call broadcast_scalar (f_fsalt_ai, master_task)
      call broadcast_scalar (f_fsalti, master_task)
      call broadcast_scalar (f_fsalti_ai, master_task)
...
!             ns1, f_albsni)
	 ...
	 call define_hist_field(n_freshi,"freshi","cm/day",tstr2D, tcstr,   &
             "freshwtr flx ice to ocn (cpl) from ice formation",            &
             "if positive, ocean gains fresh water",                    &
             mps_to_cmpdy/rhofresh, c0,                                 &
             ns1, f_freshi)
      
         call define_hist_field(n_freshi_ai,"freshi_ai","cm/day",tstr2D, tcstr, &
             "freshwtr flx ice to ocn from ice formation",                  &
             "weighted by ice area", mps_to_cmpdy/rhofresh, c0,             &
             ns1, f_freshi_ai)
	 ...
	 call define_hist_field(n_fsalti,"fsalti","kg/m^2/s",tstr2D, tcstr, &
             "salt flux ice to ocn (cpl) from ice formation",               &
             "if positive, ocean gains salt", c1, c0,                   &
             ns1, f_fsalti)
      
         call define_hist_field(n_fsalti_ai,"fsalti_ai","kg/m^2/s",tstr2D, tcstr, &
             "salt flux ice to ocean from ice formation",                     &
             "weighted by ice area", c1, c0,                                  &
             ns1, f_fsalti_ai)
...
! author:   Elizabeth C. Hunke, LANL
      ...
      use ice_flux, only: fsw, flw, fsnow, frain, sst, sss, uocn, vocn, &
          ...
          melts, meltb, meltt, meltl, fresh, fsalt, freshi, fsalti, &
          fresh_ai, fsalt_ai, freshi_ai, fsalti_ai, &
...
!                                            + awtidf*alidf(:,:,iblk), a2D)
	 ...
	 if (f_freshi  (1:1) /= 'x') &
             call accum_hist_field(n_freshi,   iblk, freshi(:,:,iblk), a2D)
         if (f_freshi_ai(1:1)/= 'x') &
             call accum_hist_field(n_freshi_ai,iblk, freshi_ai(:,:,iblk), a2D)
         if (f_fsalt  (1:1) /= 'x') &
             call accum_hist_field(n_fsalt,   iblk, fsalt(:,:,iblk), a2D)
         if (f_fsalt_ai(1:1)/= 'x') &
             call accum_hist_field(n_fsalt_ai,iblk, fsalt_ai(:,:,iblk), a2D)
	 if (f_fsalti  (1:1) /= 'x') &
             call accum_hist_field(n_fsalti,   iblk, fsalti(:,:,iblk), a2D)
         if (f_fsalti_ai(1:1)/= 'x') &
             call accum_hist_field(n_fsalti_ai,iblk, fsalti_ai(:,:,iblk), a2D)
