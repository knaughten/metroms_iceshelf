...
!          C. M. Bitz, UW
	 ...
	 freshin , & ! fresh water flux to ocean from ice formation (kg/m^2/s)
         fsaltn  , & ! salt flux to ocean (kg/m^2/s)
	 fsaltin , & ! salt flux to ocean from ice formation (kg/m^2/s)
...
! echmod: reduce size to icells?
      ...
      real (kind=dbl_kind), dimension(nx_block,ny_block) :: &
	 dhi_congel   ! ice thickness change from congelation (m/step)
	 ...
	 ! Save the fluxes from congelation separately
	 freshin(i,j) = freshin(i,j) - &
	                rhoi*dhi_congel(i,j)/dt
	 fsaltin(i,j) = fsaltin(i,j) - &
	                rhoi*dhi_congel(i,j)*ice_ref_salinity*p001/dt
	 ! Save the rest of the fluxes to the regular arrays		   
	 freshn(i,j) = freshn(i,j) + &
                       evapn(i,j) - &
                       (rhoi*(dhi-dhi_congel(i,j)) + rhos*dhs) / dt
         fsaltn(i,j) = fsaltn(i,j) - &
                       rhoi*(dhi-dhi_congel(i,j))*ice_ref_salinity*p001/dt
...
!            iage(i,j) = (iage(i,j)*hin(ij) + dt*dhi) / (hin(ij) + dhi)
         ...
         ! save thickness change from congelation
	 dhi_congel(i,j) = dhi
